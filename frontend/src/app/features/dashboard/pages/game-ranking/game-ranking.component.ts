...
export class GameRankingComponent implements OnInit, OnDestroy {
  ...
  connectSocket(): void {
    ...
    // IMPORTANTE: Enviar esProfesor: true para unirse a la sala monitor_${pin}
    this.socketService.emit('join_room', { 
      pin: this.partida?.pin || this.partidaId,
      esProfesor: this.userRole === 'profesor'
    });
    ...
    this.subs.add(
      this.socketService.on('alumno_finalizado').subscribe((data: any) => {
        ...
        if (idx !== -1) {
          // Actualizar nota si viene en los datos (solo para profesores en sala monitor)
